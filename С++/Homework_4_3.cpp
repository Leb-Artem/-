...
public:
    ...
    void preparetask(IFlightTask* ft, tStrike& p_strike) {
        ...
        if (false) {
            Singleton::self<Logger>().log(ERROR, " WRONG TIME TABLE");
        }
        alreadydone++;
        cv.notify_one();
    }
    ...
    int waitForAll(std::function<void(double)> p_callback) {
        ...
        for (const auto& it : BMlimits) {
            ...
            if (sum > Config::self().premittedBMs()) {
                Singleton::self<Logger>().log(ERROR, " TOO MUCH BMs");
                return -1;
            }
        }
        return 0;
...
int main() {
    Singleton::self<Logger>().loglevel(TRACE);
    Singleton::self<LoggerBMs>().loglevel(TRACE);
    Singleton::self<Logger>().log(INFO, " START");
    StrikeScenario ss;
    ...
    Singleton::self<Logger>().log(INFO, " CONFIG DONE");
    ...
    if (ok != 0) {
        Singleton::self<Logger>().log(ERROR, "FatalErrorInPrepare");
        return -1;
    }
    ok = ss.waitForAll([&](double perc) {Singleton::self<Logger>().log(INFO, "Completed: " + std::to_string(100 * perc)); });
    if (ok != 0) {
        Singleton::self<Logger>().log(ERROR, "FatalErrorInWaitTreads");
        return -1;
    }
    for (double t = ss.minLaunchTime(); t < ss.maxBoomTime() + cfg.stepT(); t += cfg.stepT()) {
        ...
        Singleton::self<Logger>().log(INFO, " OK");
    }
    Singleton::self<Logger>().log(INFO, " END OF PROGRAM");
}
